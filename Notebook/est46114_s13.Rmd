---
title: "Sesion 13 - Laboratorio de Analisis de Factores"
author:
...
# Modelo de factores {.tabset .tabset-fade .tabset-pills}
...
![Diagrama de dependencia del modelo de factores estatico](./MF_Estatico.png)

# Especificacion dinamica en factores {.tabset .tabset-fade .tabset-pills}
...
![Diagrama de dependencia del modelo de factores dinamico en medias](./MF_Dinamico.png)


# Especificacion dinamica en volatilidad {.tabset .tabset-fade .tabset-pills}
...
![Diagrama de dependencia del modelo de factores dinamico en volatilidad](./MF_dinamicoVol.png)
...
Hemos visto varias especificaciones del modelo de factores:

* Estatico

* Dinamico en nivel

* Dinamico en nivel y volatilidad

* Entre otras...

Adicionalmente, podemos considerar la capa de especificacion respecto a la reduccion de dimensionalidad.

**Es asi pertinente ahora estudiar como cuantificar la adecuacion del modelo ante estas especificaciones.**

### Especificacion

Grosso modo, tendremos que para un conjunto de datos
$$
\text{datos}=(\boldsymbol{x}_1,\ldots,\boldsymbol{x}_T),
$$
tedremos un conjunto de especificaciones $m=1,\ldots,M$ (siendo $M$ el numero todal de especificaciones), para las que podemos calcular la **devianza del modelo**, $DIC(m)$, definido como
$$
DIC(m)=-2 \widehat{\text{log-lik}}(\text{Modelo }m|\text{datos}) + 2\text{ep}(m),
$$
donde 

* $\widehat{\text{log-lik}}(\text{Modelo }m|\text{datos})$ denota la logverosimilitud del modelo $m$ evaluada en el **estimador puntual** (frecuentista o bayesiano) de los parametros del modelo $m$, y

* $\text{ep}(m)$ denota el numero de parametros efectivos (medias y covarianzas) del modelo $m$.

### Calculo

A partir de las estimaciones via simulacion, podemos, en general, calcular los estimados puntuales de las **medias**,
$$
\left\{\widehat{\boldsymbol{\mu}}_1,\ldots,\widehat{\boldsymbol{\mu}}_T\right\},
$$
y de la **estructura de covariabilidad**,
$$
\left\{\widehat{\boldsymbol{\Lambda}}_1,\ldots,\widehat{\boldsymbol{\Lambda}}_T\right\},
$$
y
$$
\left\{\widehat{\boldsymbol{\Sigma}}_1,\ldots,\widehat{\boldsymbol{\Sigma}}_T\right\}.
$$

De manera general, la logverosimilitud del modelo $m$ evaluada en los **estimadores puntuales** es calculada como
$$
\widehat{\text{log-lik}}(\text{Modelo }m|\text{datos}) = \sum_{t=1}^{T}\text{log}\left\{ \text{N}_p\left(\boldsymbol{x}_t|\widehat{\boldsymbol{\mu}}_t,\widehat{\boldsymbol{\Omega}}_t\right)\right\},
$$
donde 
$$
\widehat{\boldsymbol{\Omega}}_t
=\widehat{\boldsymbol{\Lambda}}_t\widehat{\boldsymbol{\Lambda}}_1'+\widehat{\boldsymbol{\Sigma}}_t,
$$
para $t=1,\ldots,T$.

**Nota.- Si se trata de un modelo reducido (estructuralmente mas simple), entonces esta logverosimilitud puede simplificarse de acuerdo a tal estructura.**
